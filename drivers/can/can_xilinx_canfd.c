 ...
 */
static enum can_state xcanfd_get_error_state(const struct xcanfd_cfg *config)
{
	uint32_t sr = xcanfd_read32(config, XCANFD_SR_OFFSET);
...
static void xcanfd_update_state(const struct device *dev, enum can_state new_state)
{
	...
	if (new_state != data->state) {
		...
		if (data->common.state_change_cb) {
			ecr = xcanfd_read32(config, XCANFD_ECR_OFFSET);
...
static void xcanfd_handle_error(const struct device *dev, const struct xcanfd_cfg *config)
{
	...
	/* Update state based on hardware status register */
	current_state = xcanfd_get_error_state(config);
...
static void xcanfd_isr(const struct device *dev)
{
	...
	if (isr & XCANFD_IXR_BSOFF_MASK) {
		...
		/* Update state and notify application */
